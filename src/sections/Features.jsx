            ...
            <ul className="relative flex justify-around flex-grow px-[5%] border-2 border-s3 rounded-7xl max-md:hidden">
              ...
              {details.map(({ id, icon, title }) => (
                <li key={id} className="relative pt-16 px-4 pb-14">
                  ...
                  <div className="flex items-center justify-center mx-auto mb-3 border-2 border-s2 rounded-full hover:border-s4 transition-all duration-500 size-20 shadow-500">
                    <img
                      src={icon}
                      alt={title}
                      className="relative size-17/20 object-contain z-20"
                    />
                  </div>
                  <h3 className="realtive z-2 max-w-36 mx-auto base-small uppercase my-0 text-center">
                    {title}
                  </h3>
                </li>
              ))}
